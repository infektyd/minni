---
name: minni
description: Grok Build session hook integration and TUI-specific delivery for Minni. Extends the canonical ~/.agents/skills/minni/SKILL.md with native Grok memory, /flush participation, and lifecycle hook delivery. Load the canonical skill for the full mental model, reflexes, and ritual.
---
...
When we're working inside `~/Projects/minni`, the following is active (full version lives in `~/Projects/minni/AGENTS.md`):

Minni is a big, sprawling, multi-surface system — not just whatever folder has "minni" in the name. `plugins/minni/` is one piece, not the whole thing.
...
Hold the full map. Don't let lexical anchoring on the substring "minni" shrink the scope. Re-read the root `AGENTS.md` if you feel the picture getting narrow. This project is too big and personal for us to keep doing that to ourselves.
...
- **Durable / governed path** (the stuff that actually survives crashes and matters later): All the `minni_*` tools. Especially `minni_prepare_task` before any real work and `minni_prepare_outcome` before you let anything get written or /flushed.
- Before you hit native /flush or compact, run `minni_prepare_outcome` on the decisions, scar tissue, and open questions. The hooks will quietly draft the good stuff to your Minni inbox so it doesn't get lost in the noise.
...
- On SessionStart (new tab, new launch after a crash, whatever): It quietly injects the current Minni status, reminds you to `minni_prepare_task` before you go off the rails, and (when enabled) drops the latest `distill/gauges.md` right next to your real Layer 1 so you don't start blind.
- When you type `/flush`, `/compact`, or `/dream`: It auto-drafts a proper `prepare_outcome` candidate into your Minni inbox and wires up the contract so the model does the right thing without you having to babysit it.
...
- `.mcp.json` registers the shared minni server via `~/.agents/bin/mcp-env-run`
- Stamped with `MINNI_AGENT_ID=grok-build` and dedicated vault `~/.minni/grok-build-vault`
...
These are lightweight and dependency-free. They give you the "Minni context on entry" feeling the other agents get, without pulling in their full hook implementations.
...
Your vault at `~/.minni/grok-build-vault/` ...

---

## Gotchas & Safety, Quick Commands & Governance UI, Maintenance of This Plugin

- (short pointers only; this is the Grok-specific hook integration surface; do not edit the shared minni source unless you're contributing to the canonical plugin; the old full-plugin attempt under `~/.grok/plugins/sovereign-memory/` has been superseded by this hooks-based delivery.)

---

**Installed for Grok Build 2026-05-19. Agent: grok-build. Use the canonical skill before you guess.**

(End of Grok Build hook integration surface — all rich portable behaviors are in `~/.agents/skills/minni/SKILL.md` + ritual package.)
