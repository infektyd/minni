...
3. Auto-grant the 26 tools: add `mcp(minni/minni_*)` to `~/.gemini/config/config.json globalPermissionGrants.allow` and `~/.gemini/antigravity-cli/settings.json permissions.allow`; drop the old `sovereign-memory/sovereign_*` grants.
