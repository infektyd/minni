...
description: "Ingest LLM Wiki pages into Minni with agent-optimized enrichment (frontmatter metadata, wikilink graph extraction, source provenance tagging). Use when setting up, maintaining, or running wiki-aware Minni ingestion."
tags: [memory, wiki, faiss, sqlite, rag, knowledge-graph]
version: 2
---

# Minni Ingestion

Wiki ingestion feeds LLM Wiki pages into Minni with agent-optimized
enrichment: frontmatter metadata baked into chunk heading context,
`[[wikilinks]]` extracted as graph edges in `memory_links`, and source
provenance tagging.
...
# Agent context
python agent_api.py <agent_id> --context
```
