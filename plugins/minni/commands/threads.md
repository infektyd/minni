...
- **Host dispatch (Wave 3, first wet set)**: After claim, `dispatchWorkerPacket` takes one WorkerPacket. grok worker-start is MISSING. agy default allowlist is a typed CANNOT (`minni_thread_worker_update` is not granted). A named agy worker allowlist exists as `minni_thread_worker_update` only and is still not wet (`spawned` false). Codex maps that packet onto one subagent and marks dispatch UNPROVEN. `spawned` is always false. Cursor is out of this set. The adapter does not spawn or wake. G3 is a notification relay (delivery cursors, rebuild-from-journal-seq, fail-closed cursor). Immediate wake is unsupported; hooks are the deferred fallback and do not poll `minni_thread_events`. Automatic spawning and grok worker-start are not implemented. `GROK_WORKER_START` stays null. `spawned` stays false.
