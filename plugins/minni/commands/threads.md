...
8. When scope changes materially, call `minni_thread_replan` with either a full set of `new_slices` or differential updates (`add_slices` and/or `drop_slice_ids`) instead of editing the vault note by hand. History is preserved via `superseded` slices. Orch can remount named live `depends_on` via `set_depends_on` without a full-set `new_slices` rewrite; unnamed live slices stay. Remount targets must exist, not be superseded, and must not be the remounted slice. `new_slices`/`add_slices` must not write a self-edge; `depends_on` targets must exist and not be superseded; a same-call exclusive split must not write a waiter on the dead parent.
9. Follow progress and synchronize state via ordered event cursors with `minni_thread_events`:
   - Call `minni_thread_events(plan_id?, since_seq?, limit?)` to read append-only journal events recorded after `since_seq`. Treat `journal_truncated` / `cursor_gap` as a hole (`last_dropped_seq` + `first_kept_seq`); do not assume seq is contiguous.
...
- **Library adapters (not MCP tools)**: `buildWorkerPacketAfterClaim` after assign → claim; `dispatchWorkerPacket` for host honesty; `thread-notification-relay` for G3 delivery cursors / pending attention. Do not register any of these as a tool.
...
- **Host dispatch (Wave 3, first wet set)**: After claim, `dispatchWorkerPacket` takes one WorkerPacket. grok worker-start is MISSING. agy default allowlist is a typed CANNOT (`minni_thread_worker_update` is not granted). A named agy worker allowlist exists as `minni_thread_worker_update` only and is still not wet (`spawned` false). Codex maps that packet onto one subagent and marks dispatch UNPROVEN. `spawned` is always false. Cursor is out of this set. The adapter does not spawn or wake. G3 is a notification relay (plugin `cursors.json` on journal append, rebuild-from-journal-seq, fail-closed cursor; SQLite 020 unused). Immediate wake is unsupported; hooks are the deferred fallback and do not poll `minni_thread_events`. Automatic spawning and grok worker-start are not implemented. `GROK_WORKER_START` stays null. `spawned` stays false.
