...
Call `minni_prepare_outcome` with:
...
This is dry-run only — nothing is written. Review the `outcomeDraft.learnCandidates`. If any are worth committing, follow up with `/minni:learn`. The draft's `doNotStore` list is non-negotiable: never commit those even if asked.
