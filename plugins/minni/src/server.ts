...
server.registerTool(
  "minni_thread_replan",
  {
    ...
    description:
      "Replan preserving slice history: supersede dropped non-final slices, append new proposals, persist + journal. plan_id defaults to the active plan. Worker propose_structure does not apply. Orch apply is add/drop, not a kind enum: expand = add_slices only (proposer stays); split = drop_slice_ids of the claimed parent + add_slices children (no parent-id reuse; dependents of the replaced parent stay blocked until orch remounts named depends_on onto the children via set_depends_on — unnamed live slices stay; remount targets must exist and not be superseded); contract = drop_slice_ids only (drop-without-replacement still unblocks dependents). Remount-only set_depends_on is valid without new_slices. Drop supersedes; it never deletes.",
    inputSchema: {
      plan_id: z.string().min(1).optional(),
