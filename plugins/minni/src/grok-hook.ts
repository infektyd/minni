...
function workspaceFromPayload(payload: Record<string, unknown>): string {
  return workspaceFromPayloadShared(payload, GROK_WORKSPACE_ID);
}
...
async function handleSessionStart(payload: Record<string, unknown>): Promise<HookOutput> {
  ...
  const expiredHandoffs = await expireStaleInboxHandoffs(GROK_VAULT_PATH);
  const [status, tail, identityRead, inboxStatus] = await Promise.all([
    ...
    readInboxStatus(GROK_VAULT_PATH, 3),
  ]);
