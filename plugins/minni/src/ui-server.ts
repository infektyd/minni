...
function consoleAuthRequired(pathname: string): boolean {
  // /api/health is the only unauthenticated route (liveness probe, no vault data).
  ...
  if (consoleAuthAutoGenerated) {
    console.log(
      "Minni console auth: MINNI_CONSOLE_TOKEN was not set — generated a session token.\n" +
        `  Send this header with console requests: Authorization: Bearer ${effectiveConsoleAuth}\n` +
