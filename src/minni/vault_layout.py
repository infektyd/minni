...
the learning corpus. Daemon learnings live in shared ``~/.minni/learnings``
and the AFM wiki ``~/.minni/vault``; an agent vault that is ``.index``-only
is a missing inbox/identity layout, not a hole those learnings
should fill. The plugin's ``ensureVault`` already creates this layout on
first hook; CLI/socket principals never took that path. Seed
only when the root already exists — do not invent a vault from a missing
path. Do not route learnings into an agent vault.
"""
