...
def cmd_index(args):
    ...
    from minni.index_all import index_all
...
def cmd_query(args):
    from minni.agent_api import SovereignAgent
    if not args:
...
def cmd_graph(args):
    from minni.graph_export import GraphExporter
    agent_filter = None
    if "--agent" in args:
...
def cmd_faiss(args):
    ...
    if "--rebuild" in args or len(args) == 0:
        ...
        import os
        from minni.faiss_persist import _faiss_dir_for_db, compute_db_checksum
...
def cmd_vectors(args):
    ...
    from minni.vector_sync import get_backend_state, sync_all
