...
from minni.eval.harness import (
    _MockSearcher,
    RipgrepSearcher,
    _calibration_error,
    _extract_doc_ids,
...
class TestUnknownKwargs:
    ...
    def test_expand_kwarg_is_forwarded(self, caplog):
        ...
        from minni.eval.harness import _safe_search
...
class TestRecordMode:
    ...
    def test_record_appends_to_file(self, tmp_path, monkeypatch):
        ...
        from minni.eval.harness import cmd_record
        import argparse

        p = tmp_path / "queries.jsonl"
        monkeypatch.setattr("minni.eval.harness._queries_path", lambda: p)
