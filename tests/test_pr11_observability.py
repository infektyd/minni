...
def test_writeback_import_is_lightweight_without_numpy():
    # v0.2 src layout: the package is installed in the venv, so the subprocess
    # imports minni.writeback directly (no sys.path splice). The codemod cannot
    # rewrite imports inside string literals, so this one is by hand.
    script = (
        "import sys; "
...
def test_trace_redaction_applied_via_redact_value():
    ...
    """
    import minni.minnid as minnid
    from minni.minnid import _redact_value  # type: ignore
