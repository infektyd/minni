...
def test_session_distill_emits_additive_review_draft(tmp_path, monkeypatch):
    from minni.afm_passes.session_distillation import run
    ...
    monkeypatch.setattr("minni.afm_provider.invoke_native_afm", _route(op_map))
...
def test_similar_existing_learning_reads_selected_column_directly(tmp_path):
    from minni.afm_passes.session_distillation import _similar_existing_learning
...
def test_session_distill_chunks_and_reduces_oversized_text(tmp_path, monkeypatch):
    from minni.afm_passes import session_distillation
...
def test_triage_advisory_fold_preserves_redact_from_any_chunk(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MINNI_AFM_MODE", "native")
    monkeypatch.setattr("minni.afm_provider.invoke_native_afm", _route({"triage": fake_triage}))
...
def test_entity_extract_cap_logs_dropped_count(tmp_path, monkeypatch, caplog):
    ...
    from minni.afm_passes.session_distillation import _native_entity_extract
    ...
    monkeypatch.setenv("MINNI_AFM_MODE", "native")
    monkeypatch.setattr("minni.afm_provider.invoke_native_afm", _route({"entity_extract": fake_entity_extract}))
