...
- Thread lock Q is dump-and-return, not sit-and-wait: when the plan lock is
  ...
  In-process kick yields to exclusive replan: exclusive replan reserves before the persist lock, and kick/drain does not apply an accepted start while that reservation is live. A live accepting process can stay up; journal has no `slice.started` on a parent that exclusive split then supersedes. After supersede, leftover Q still drops. Kick still drains when no exclusive replan is in flight. `THREAD_BUSY` stays overflow, not the default. No SIGKILL required. No new MCP tool.
