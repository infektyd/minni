...
- **P3 — Vault + daemon migration** (daemon already DOWN for the whole migration): backup → move `~/.sovereign-memory` → `~/.minni` (clean rename, no symlink-back) → rename `sovereign_memory.db` → `minni.db`. **Obligations surfaced by P2 (must complete before bring-up):**
  - **Migrate ~32 existing vault notes' frontmatter** `sovereign_learning:` → `minni_learning:` (P2 renamed the TS read/write to `minni_learning`; Python does not use this field, but un-migrated notes lose their +2 ranking boost). Mechanical, scoped to `~/.minni/*/wiki`.
  - **Rename the 4 daemon JSON-RPC method strings** still on `sovereign_*` in `plugins/minni/src/sovereign.ts` (`sovereign_ack_handoff`, `sovereign_list_pending_handoffs`, `sovereign_await_handoff`, `sovereign_subscribe_contradictions`) **atomically with the Python daemon handlers** that serve them — these are the wire protocol, so TS and `engine/` must change together.
  - **Rewrite the launchd plist** `com.openclaw.sovrd.plist`: `SOVEREIGN_*` env → `MINNI_*`, `SOVEREIGN_WORKSPACE_ID=~/Projects/sovereignMemory` → `~/Projects/minni` (anchor 1), DB/socket paths → `~/.minni/…`/`minni.db`, ProgramArguments paths, and rename the job label `com.openclaw.sovrd` → `com.openclaw.minnid` if desired.
  - **Bring-up** (final step, operator present): restart daemon as `minnid` on `~/.minni`, reinstall plugins, verify recall/learn across all platforms.
