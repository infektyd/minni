...
| `learn` | agent | Writes a row to `learnings`. | Stores a durable learning keyed by `agent_id` and `category`. |
...
| `list_candidates` | agent | None | G14/G17: list staged candidates for the stamped principal (console/governance view). Defaults to `status=proposed`; responses are POLICY §2 redacted and include `total`/`has_more`. |
